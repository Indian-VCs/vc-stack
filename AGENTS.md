# AGENTS.md — project memory for VC Stack

Context for future Codex (or any AI) sessions working on this repo. Keeps you from re-discovering decisions that are already locked in.
...
3. **`src/lib/db/schema.ts`** — Drizzle schema for the **admin backend** (Cloudflare D1 in prod, SQLite in dev).
4. **`src/app/(main)/*`** — all public routes. `(main)` is a route group, not part of the URL.
5. **`src/app/admin/*`** — admin portal. Gated by [`src/middleware.ts`](src/middleware.ts) + per-page `requireAdmin()`.
...
- **Public** (`src/app/(main)/*`): reads from D1 via `src/lib/db/queries.ts`. Falls back to `src/lib/static-catalog.ts` when D1 is empty or unreachable. The wrappers live in `src/lib/data.ts`.
- **Admin** (`src/app/admin/*`): reads and writes the same D1 tables. Edits propagate to the public site via `revalidatePath`.
- The fallback is load-bearing: `next build` runs before D1 is bound, so the prerender drops to STATIC. Don't delete `static-catalog.ts` — it's also the seed source for `scripts/seed.ts`.

### Admin portal — how it's gated
- iron-session cookies (encrypted + signed) — no constant string passwords, no forgable values.
- bcrypt password verify (env-based single admin: `ADMIN_EMAIL` + `ADMIN_PASSWORD_HASH`).
- IP-based rate limit (5 attempts / 15 min) on the login endpoint.
- `src/proxy.ts` (Next.js 16 convention; the file MUST be `proxy.ts` exporting `proxy()` — `middleware.ts` is deprecated and emits a build warning). Gates every `/admin/*` and `/api/admin/*` request.
- Defense-in-depth: every admin page calls `requireAdmin()`, every server action calls `requireAdminAction()`.
- Every mutation writes to the `audit_log` table.

Full operational docs in [`ADMIN.md`](ADMIN.md). Setup in [`SETUP.md`](SETUP.md).

### Security headers
`next.config.ts` exports `headers()` with CSP, HSTS, X-Frame-Options: DENY, X-Content-Type-Options, Referrer-Policy, Permissions-Policy, COOP. CSP allows GTM/GA/Google favicons/Webflow CDN/Substack iframes. Tune carefully — test in `Content-Security-Policy-Report-Only` first if changing.
...
```
Public site runs on static data. Substack URL is a public constant in `src/lib/substack.ts`.

### Admin backend (Cloudflare Workers + D1)
Full setup in [`SETUP.md`](SETUP.md). Required Cloudflare secrets: `ADMIN_EMAIL`, `ADMIN_PASSWORD_HASH`, `SESSION_PASSWORD`. D1 database `vc-stack` bound as `DB` in `wrangler.jsonc`. Migrations in `drizzle/migrations`.
...
- Multi-admin via `admin_users` table (schema is ready, login route still env-based).
