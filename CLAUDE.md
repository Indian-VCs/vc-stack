...
- **iCloud Drive eats `.next/dev/` if the project sits under `~/Documents`**: macOS file-provider syncs everything inside `~/Documents` to iCloud. iCloud silently empties `.next/dev/` mid-session — you'll see `routes-manifest.json` / `middleware-manifest.json` ENOENT errors and 500s on every route while the dev server appears alive. The `npm run dev` script works around it by setting `NEXT_DIST_DIR=../../.cache/vcstack-next` (build cache lands at `~/.cache/vcstack-next`, outside iCloud), `NODE_PATH=$PWD/node_modules` (so compiled chunks at `~/.cache/...` can still resolve `react/jsx-runtime` etc.), and `--webpack` (Turbopack is even more sensitive to this race). Two caveats: Next.js 16 has a bug where **absolute** `distDir` paths are treated as project-relative — use the `../../.cache/...` relative form. And `tsconfig.json` already includes the cache-types globs so Next stops trying to auto-modify the file. The proper long-term fix is moving the project out of `~/Documents` (e.g. `~/code/vc-stack`).
